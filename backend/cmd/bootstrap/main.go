...
func main() {
	...
	demoMessages := []struct {
		...
		{message.RoleAssistant, "Here's the equivalent FortiSASE configuration:\n\n**1. Onboarding / Connectivity:**\n- Enroll the 10.0.0.0/24 site via FortiClient or an IPsec tunnel to the nearest FortiSASE PoP.\n- Traffic steering is handled by the FortiSASE data center, replacing Zscaler ZCC/GRE tunneling.\n\n**2. Firewall Policy (FortiOS syntax under FortiSASE):**\n```config firewall policy\n edit 1\n  set srcintf \"ssl-root\"\n  set dstintf \"virtual-wan\"\n  set srcaddr \"LAN_10.0.0.0_24\"\n  set dstaddr \"all\"\n  set action accept\n  set schedule \"always\"\n  set service \"HTTP\" \"HTTPS\" \"DNS\"\n  set logtraffic all\n  set utm-status enable\n  set webfilter-profile \"default\"\n  set dnsfilter-profile \"default\"\n  set ips-sensor \"default\"\n next\nend```\n\n**3. Web Filtering:**\n- The Zscaler `Corporate` URL category profile maps to a FortiSASE web filter profile applied via `set webfilter-profile`.\n\n**4. DNS Control:**\n- Zscaler DNS Resolver inspection maps to FortiSASE DNS Filter profile (`set dnsfilter-profile \"default\"`).\n\nNotes:\n- Zscaler's cloud-native firewall rule (source IP / service / action) maps directly to a FortiGate-style policy in FortiSASE, but you gain UTM profiles (IPS, web filter, DNS filter) on the same policy.\n- Zscaler's `Log: Enabled` maps to `set logtraffic all`.\n- Zscaler's `Destination: Any` becomes `set dstaddr \"all\"` — tighten this if you want stricter egress control."},
